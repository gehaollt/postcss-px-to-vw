...
const postcssPxToViewport = (options) => {
  ...
  const mutiDesignRules = opts.mutiDesignWidth?.map((rul) => {
    return {
      ...rul,
      rules: [],
    };
        ...
        if (opts.mutiDesign && !rule.parent?.params) {
              ...
              mutiDesignRule
                ...
                ?.rules.append(
                  decl.clone({
                    value: decl.value.replace(
                      pxRegex,
                      (0, _utils.createPxReplace)(opts, opts.mutiDesignUnit, width),
                    ),
                  }),
                );
            });
          ...
          mutiDesignRule?.forEach((ru) => {
            if (ru.rules.nodes.length > 0) {
              mutiDesignRules.find((ri) => ri.value === ru.value)?.rules.push(ru.rules);
            }
          });
        }
